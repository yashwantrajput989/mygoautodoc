          ...
          {providers.map((p) => {
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center">
                      <input
                        ...
                        className="w-4 h-4 text-primary focus:ring-primary border-border cursor-pointer bg-background"
                      />
                    </div>
                    <label htmlFor={`provider-${p.id}`} className="cursor-pointer">
                      <div className="flex items-center gap-2">
                        <div className={cn("w-2.5 h-2.5 rounded-full", p.color)} />
                        <span className="font-bold text-foreground text-sm">{p.label || p.id}</span>
                      </div>
                      <p className="text-[11px] text-muted-foreground font-medium mt-0.5">{p.description}</p>
                    </label>
                  </div>

                  <div className="flex items-center gap-3 flex-1 max-w-xl md:justify-end">
                    {/* API Key Input */}
                    <div className="relative flex-1 max-w-sm">
                      <input
                        ...
                        className="w-full bg-background border border-border h-9 px-3 pr-8 rounded-lg text-xs focus:ring-1 focus:ring-primary outline-none transition-all font-mono"
                      />
                      <button
                        onClick={() => toggleVisibility(p.id)}
                        className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"
                      >
                        {visibility[p.id] ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-primary text-primary-foreground text-xs font-bold rounded-lg hover:bg-primary/95 disabled:opacity-50 transition-all flex items-center gap-1 h-9 shadow-sm"
                    >
                ...
                {hasKey && (
                  <div className="mt-4 pl-7 border-t border-border/40 pt-4 flex flex-col sm:flex-row sm:items-center gap-4 animate-in fade-in duration-300">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setModels({ ...models, [p.keyName]: newM });
                            fetch(`${API_BASE}/settings/ai`, {
                              method: "POST",
                              headers: { "Content-Type": "application/json" },
                              body: JSON.stringify({
                                provider: p.id,
                                api_key: keys[p.keyName as keyof typeof keys],
                                model: newM,
                              }),
                            }).then((res) => {
                              if (res.ok) {
                                toast.success(`${p.id} model updated to ${newM}`);
                              }
                            });
                          }}
                          className="bg-background border border-border h-9 px-3 rounded-md text-xs focus:ring-1 focus:ring-primary outline-none transition-all w-60 text-foreground font-semibold"
                        >
                        ...
                        <button
                          ...
                          className="p-2 border border-border bg-card hover:bg-muted text-muted-foreground hover:text-foreground rounded-md transition-all flex items-center justify-center h-9"
                          title="Fetch latest models"
                        >
                          <RefreshCw className={cn("h-3.5 w-3.5", isLoadingModels[p.keyName] && "animate-spin")} />
                        </button>
                      </div>
                    </div>
                  </div>
